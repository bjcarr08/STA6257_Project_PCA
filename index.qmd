...
[**Given a sample of data: ** ${\{x_1}, ... , {x_n}\}, where\ {x_i} = {({x_{i1}}, ... , {x_{ip}})}^T$]{style="color: #8B814C;"}

[**We will estimate the sample mean vector $\overline{x} = \hat{\mu}$ and the sample cov matrix $S = \hat{\sum}$**]{style="color: #8B814C;"}
...
[PCA results depend on the scale or units of the variables so, for unscaled data, calculations should either be preformed on the correlation matrix (instead of the covariance matrix) or the data should be standardized with mean 0 and variance 1 (z scores). [@lang2021applied]]{style="color: #8B814C;"}
