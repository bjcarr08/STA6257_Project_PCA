...
Principal Component Analysis (PCA) is an unsupervised machine learning method (@fig-machineLearning) that can be used as the first step in data analysis. The main goal of PCA is to reduce the number of dimensions/features while retaining most of the original variability. Reducing the number of features makes it easier to graphically visualize hidden pattern in the data, it's also useful when encountering a high number of predictor variables or highly correlated variables when running a regression model [@lang2021applied]. Using a clustering algorithm, such as k-means, after data is transformed using PCA is a powerful method to use on high dimensional data containing linearly correlated features [@Alkha]. 
...
## [**4. Conclusion**]{style="color: #191970;"}
